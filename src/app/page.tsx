...
export default function Home() {
  ...
  const email = useMemo(() => `mailto:${business.email}?subject=${encodeURIComponent(`${occasion} tea party enquiry`)}&body=${encodeURIComponent(`Hi Dotty's,\n\nI'm interested in a ${occasion.toLowerCase()} afternoon tea for ${guests} guests.\n\nDate / details:\n\nThanks!`)}`, [occasion, guests]);

  return (
    <main className="min-h-screen bg-[#fbf3e5] text-[#3c2a20] pb-16 selection:bg-[#b9784d] selection:text-white">
      <nav className="sticky top-0 z-50 border-b border-[#3c2a20]/10 bg-[#fbf3e5]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-10">
          <a href="#top" className="font-serif text-xl font-semibold tracking-[-.04em]">DOTTY'S <span className="text-[#b9784d]">TEAS</span></a>
          <div className="hidden gap-8 text-sm text-[#3c2a20]/60 md:flex"><a href="#experience">The experience</a><a href="#occasions">Occasions</a><a href="#gallery">Gallery</a><a href="#contact">Contact</a></div>
          <a href={email} className="rounded-full bg-[#3c2a20] px-5 py-2.5 text-sm font-semibold text-[#fbf3e5]">Plan your tea</a>
        </div>
      </nav>

      <section id="top" className="relative overflow-hidden px-5 py-20 lg:px-10 lg:py-28">
        <div className="absolute right-[-10%] top-[-8%] h-[32rem] w-[32rem] rounded-full bg-[#d9a06f]/25 blur-3xl" />
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
          <div className="relative z-10">
            <p className="text-xs uppercase tracking-[.35em] text-[#b9784d]">Vintage afternoon tea · Eastbourne & East Sussex</p>
            <h1 className="mt-7 font-serif text-7xl leading-[.82] tracking-[-.06em] sm:text-8xl lg:text-[7.5rem]">Tea,<br/><em className="text-[#b9784d]">but make it</em><br/>an occasion.</h1>
            <p className="mt-8 max-w-xl text-lg leading-8 text-[#3c2a20]/65">Dotty's brings homemade scones, cakes, delicate sandwiches and beautiful vintage tableware to your door — perfect for birthdays, hens, baby showers and days worth celebrating.</p>
            <div className="mt-9 flex flex-col gap-3 sm:flex-row"><a href="#occasions" className="rounded-full bg-[#b9784d] px-8 py-4 text-center font-semibold text-white">Plan your tea party</a><a href="tel:+441323485909" className="rounded-full border border-[#3c2a20]/20 px-8 py-4 text-center font-semibold">Call 01323 485909</a></div>
          </div>
          <div className="relative min-h-[600px] overflow-hidden rounded-[2.5rem] border-8 border-white shadow-2xl shadow-[#6e4933]/10"><div className="absolute inset-0 bg-cover bg-center" style={{backgroundImage:`url(${media.hero.src})`}}/><div className="absolute bottom-6 left-6 rounded-full bg-white/90 px-5 py-3 text-xs uppercase tracking-[.2em] text-[#3c2a20]">Demo photography · replace before launch</div></div>
        </div>
      </section>
      ...
      <section id="occasions" className="px-5 py-28 lg:px-10"><div className="mx-auto max-w-7xl"><div className="mb-12"><p className="text-xs uppercase tracking-[.35em] text-[#b9784d]">Choose your occasion</p><h2 className="mt-4 font-serif text-6xl tracking-[-.05em] sm:text-8xl">Let's make<br/><em className="text-[#b9784d]">a plan.</em></h2></div><div className="grid gap-10 lg:grid-cols-[1fr_.8fr]"><div className="grid gap-4 sm:grid-cols-2">{occasions.map(([title,desc])=><button key={title} onClick={()=>setOccasion(title)} className={`rounded-[1.5rem] border p-7 text-left transition ${occasion===title?"border-[#b9784d] bg-[#b9784d] text-white shadow-xl":"border-[#3c2a20]/10 bg-white/60 hover:-translate-y-1"}`}><span className="text-2xl">{occasion===title?"♥":"♡"}</span><h3 className="mt-12 font-serif text-3xl">{title}</h3><p className={`mt-3 text-sm leading-6 ${occasion===title?"text-white/75":"text-[#3c2a20]/50"}`}>{desc}</p></button>)}</div><div className="rounded-[2rem] bg-[#3c2a20] p-8 text-[#fbf3e5] sm:p-10"><p className="text-xs uppercase tracking-[.3em] text-[#e1b18c]">Your tea party</p><h3 className="mt-5 font-serif text-4xl">{occasion}</h3><label className="mt-10 block text-sm text-white/60">Approximate guests</label><select value={guests} onChange={e=>setGuests(e.target.value)} className="mt-2 w-full rounded-xl border border-white/15 bg-white/10 px-4 py-4 text-white outline-none"><option className="text-black">2–4</option><option className="text-black">5–7</option><option className="text-black">8–12</option><option className="text-black">13+</option></select><p className="mt-6 text-sm leading-6 text-white/55">Tell Dotty what you're planning and start the conversation. No payment is taken here.</p><a href={email} className="mt-8 block rounded-full bg-[#e1b18c] px-6 py-4 text-center font-semibold text-[#3c2a20]">Email Dotty about this →</a></div></div></div></section>
