...
export default function Home() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#050505] text-white selection:bg-amber-400 selection:text-black">
      ...
      <section className="relative flex min-h-screen items-center px-6 pt-28 lg:px-10" style={{ background: `radial-gradient(circle 420px at ${position.x}% ${position.y}%, rgba(245,158,11,0.13), transparent 70%)` }}>
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_0%,#050505_75%)]" />
        <div className="relative mx-auto w-full max-w-7xl">
          <div className="mb-8 flex items-center gap-3 text-xs uppercase tracking-[0.35em] text-white/35"><span className="h-px w-10 bg-amber-400/60" />{business.tagline}</div>
          ...
          <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_340px] lg:items-end">
            <p className="max-w-xl text-lg leading-8 text-white/45 sm:text-xl">Professional service from a local business that genuinely cares about the details, the experience and the final result.</p>
            <div><p className="mb-4 text-xs uppercase tracking-[0.3em] text-white/25">The standard</p><p className="text-3xl font-medium tracking-tight">Beautiful <span className="text-amber-400">results.</span></p></div>
          </div>
          <div className="mt-12 flex flex-col gap-4 sm:flex-row">
            ...
            <a href="#services" className="rounded-full border border-white/10 px-8 py-4 text-center font-medium text-white/70 transition hover:border-white/25 hover:bg-white/[0.04] hover:text-white">Discover what we do ↓</a>
          </div>
        </div>
      </section>
      ...
      <section id="services" className="border-t border-white/[0.07] px-6 py-32 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <div className="mb-20 flex flex-col justify-between gap-8 md:flex-row md:items-end"><div><p className="text-xs uppercase tracking-[0.35em] text-amber-400">What we do</p><h2 className="mt-5 max-w-2xl text-5xl font-semibold tracking-[-0.05em] sm:text-7xl">Less ordinary.<br /><span className="text-white/25">More memorable.</span></h2></div><p className="max-w-xs text-sm leading-6 text-white/30">A carefully considered service from first contact to final result.</p></div>
          <div className="grid border-y border-white/[0.08] md:grid-cols-3 md:divide-x md:divide-white/[0.08]">
            {serviceCards.map(([number, title, text]) => <article key={number} className="group min-h-[330px] border-b border-white/[0.08] p-8 transition duration-500 hover:bg-white/[0.025] md:border-b-0 md:p-10"><div className="flex items-start justify-between"><span className="text-sm text-amber-400/60">{number}</span><span className="text-xl text-white/10 transition duration-500 group-hover:text-amber-400">↗</span></div><div className="mt-24"><h3 className="text-2xl font-medium tracking-tight transition group-hover:text-amber-400">{title}</h3><p className="mt-4 text-sm leading-7 text-white/35">{text}</p></div></article>)}
          </div>
        </div>
      </section>
